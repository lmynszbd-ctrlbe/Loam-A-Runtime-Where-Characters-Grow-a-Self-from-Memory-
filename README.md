...
- **反滚雪球机制**：人格变化只锚定原始对话，不锚定“上一版总结”。
- **强制原文入库**：每轮 user+assistant 原文强制写入，不吃“工具调用概率”。
- **多上游单入口**：Agent 只填一个 URL，通过 `provider/model` 切多家中转。
- **双模型解耦**：前台聊天模型与后台生长模型独立配置，互不绑定。
- **可审计可重建**：任意阶段变化都能追溯到经历，派生层可重算。
