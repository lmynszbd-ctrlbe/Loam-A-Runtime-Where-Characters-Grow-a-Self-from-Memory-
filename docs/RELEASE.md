...
Single `docs/DEPLOY.md` covers Termux, Windows, macOS, Linux, Docker, systemd, and MCP tool integration. All platforms share the same upstream mapping mechanism via `~/.loam/upstreams.json` and digestion config via `~/.loam/secrets.json`.
